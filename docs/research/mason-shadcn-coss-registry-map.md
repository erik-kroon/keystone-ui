...
- [Issue #26: Map shadcn and coss registry patterns for Mason](https://github.com/erik-kroon/keystone-ui/issues/26)
...
Mason should adopt the shadcn registry mental model: source-first registry items, project config, aliases, dependency metadata, registry dependencies, static JSON output, docs previews, and CLI install plans. Mason should adapt those patterns for Solid project detection, Solid JSX, SolidStart and TanStack Router conventions, Keystone-backed behavior imports, and generated output typecheck/build gates.

Mason should treat coss as a later UI styling and component-inventory reference, not as a registry architecture authority. coss is useful for token discipline, component grouping, dense primitive coverage, docs reference shape, and registry consumption patterns. It remains React/Base UI-oriented, so Mason should not copy its runtime component code directly.
...
- Replace Radix-backed behavior imports with Keystone imports. Mason components should style and compose Keystone primitives, not reimplement behavior.
...
- Keep the shadcn item vocabulary, but prefer Mason names where clearer: `registry:ui`, `registry:block`, `registry:hook`, `registry:lib`, `registry:theme`, `registry:page`, `registry:template`, `registry:config`, `registry:rule`, `registry:asset`, and `registry:file`.
...
- React Server Components flags such as `rsc` as a Mason driver. Mason may need SSR metadata, but it should describe Solid SSR/hydration support instead.
...
- Silent style fallbacks that hide project mismatch. Mason should report unsupported style/router/project shapes clearly.
- Broad compatibility with every shadcn internal item type before Mason proves core UI, lib, hook, theme, config, and file installs.
...
- Use Bun scripts in the monorepo, for example `mason registry build` and `mason registry validate`.
- Author styles under `registry/default/...` first. Add style variants only after the default style proves component quality.
- Prefer Mason item types in source JSON. Accept shadcn item JSON as an import path if validation can normalize it safely.
...
- Registry tags for search and categorization. Mason should maintain a typed tag set before community registry indexing.
...
- Replace coss React/Base UI components with Solid/Keystone-backed components.
- Use `data-scope` and `data-part` from Keystone as stable styling selectors. Mason can add its own data attributes only when styling needs them.
- Token names can be similar, but Mason should keep tokens small until the first components prove the system.
- coss's large component inventory should become a prioritization reference, not immediate scope.
- Portal guidance should map to Keystone overlay internals and Solid app roots.
...
- Treating Base UI portal or React composition APIs as Mason runtime constraints.
- Importing coss token volume wholesale before Mason has a real default style and docs surface.
- Building hundreds of component items before Keystone primitives are stable enough to support them.

## React And Next Assumptions That Must Not Drive Mason
...
## Solid-Specific Mason Adaptations
...
- Components import Keystone primitives for behavior and styling hooks via stable `data-scope`, `data-part`, and CSS variables.
...
## Mason Modules To Change Next

1. `packages/mason-registry/src/schema.ts`
   - Add or confirm compatibility metadata for Mason, Solid, Keystone, framework, and style system.
...
8. Future `registry/default`
   ...
   - Next add `dialog` to prove Keystone-backed behavior, overlay styling, SSR/hydration, accessibility, and generated output checks.
...
- coss is a later Mason UI style and component inventory reference, not a registry schema source of truth.
- Keystone remains the behavior layer. Mason owns styled source, registry metadata, CLI workflows, and docs previews.
