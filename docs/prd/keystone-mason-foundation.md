# PRD: Keystone And Mason Foundation
...
- Mason: a copy-paste UI registry, CLI, blocks, themes, templates, and documentation layer built on Keystone.

Keystone should handle the hard interaction and accessibility details that teams do not want to rebuild: controlled and uncontrolled state, focus management, keyboard interaction, dismissal, layering, portals, positioning, collections, typeahead, forms, SSR, hydration, and stable DOM contracts.

Mason should install readable Solid source files into user projects. Mason components should use Keystone primitives for behavior, while giving users ownership of styled component source. Mason should also provide blocks, templates, theme tokens, registry validation, CLI project detection, diffs, updates, and publishing workflows.
...
1. As a Solid app developer, I want to initialize Mason in an existing app, so that I can add polished UI without manually wiring project conventions.
2. As a Solid app developer, I want to add a dialog component from Mason, so that I can use accessible modal behavior with editable styled source.
3. As a Solid app developer, I want Mason components to import Keystone behavior, so that accessibility and interaction fixes can be updated centrally.
...
5. As a Solid app developer, I want Mason to detect my package manager, framework, TypeScript setup, styles, aliases, and router where possible, so that setup requires minimal manual configuration.
6. As a Solid app developer, I want Mason to support dry runs and diffs, so that I can understand file changes before accepting them.
7. As a Solid app developer, I want Mason to warn before overwriting local changes, so that I do not lose customizations.
...
11. As a Solid app developer, I want Keystone-backed components to avoid hydration warnings, so that SolidStart and SSR projects remain reliable.
...
15. As a Solid app developer, I want Mason docs to show install commands, usage, source, dependencies, and customization notes, so that I can adopt components confidently.
...
38. As a docs reader, I want Mason documentation organized around CLI, components, blocks, themes, templates, registry, publishing, customization, framework support, monorepos, migration, and troubleshooting, so that I can solve practical adoption problems.
39. As a maintainer, I want Keystone and Mason package boundaries to be explicit, so that future contributors do not accidentally couple primitive behavior to styled registry output.
...
42. As a maintainer, I want Mason to use Solid-specific project detection and transforms, so that it is not limited by React-oriented tooling.
...
- Keystone and Mason will be separate product layers in one monorepo.
...
- Mason will own CLI workflows, project detection, registry schema, registry validation, styled source generation, blocks, templates, themes, previews, and publishing workflows.
- Keystone must not depend on Mason.
- Mason components should depend on Keystone primitives for behavior by default.
- Keystone should start with internal kernel modules before visible component breadth.
- Keystone should expose compound component APIs for app and wrapper authors.
- Keystone should expose low-level creator APIs for primitive power users and advanced wrappers.
...
- Keystone should use a Solid-native polymorphic `as` API, including callback-style advanced usage.
...
- Keystone must avoid direct browser access during SSR and avoid hydration-unsafe ID generation.
...
- Keystone kernel modules should have focused unit tests for controllable state, event composition, collections, typeahead, focus stacks, dismissal stacks, presence, and floating adapters.
- Keystone primitive tests should cover keyboard interaction, focus restore, outside interaction, nested overlays, disabled states, RTL, form submission, and controlled/uncontrolled behavior.
...
- Mason generated-code tests should install registry items into example apps, typecheck them, build them, and run smoke interactions.
- Visual regression tests should focus on Mason components, blocks, templates, and docs previews.
...
- A Keystone dependency on Mason.
- A Mason implementation that reimplements focus traps, select behavior, typeahead, or accessibility behavior instead of using Keystone.
...
Keystone and Mason are working names. Package scope and naming clearance should happen before public launch.

The recommended build order is:

1. Keystone internals.
2. Keystone overlays.
3. Keystone fields and forms.
4. Keystone select and combobox.
5. Mason CLI and registry.
6. Mason core components.
7. Mason blocks and templates.
8. Community registry support.

The current repository is still a small scaffold. The next planning artifacts should be a Keystone API RFC, Mason registry RFC, and accessibility testing plan.
