# PRD: Core Internals Inspiration Parity
...
Build a Core internal kernel that reaches parity with the relevant architectural lessons from Base UI and Kobalte before expanding the primitive catalog.
...
3. As a Keystone maintainer, I want Kobalte treated as the primary Solid-native reference, so that Core APIs fit Solid rather than React.
...
19. As a primitive implementer, I want geometry CSS variables, so that UI and design-system wrappers can animate and size floating content.
...
28. As a UI item author, I want Core internals to expose stable data and CSS contracts, so that UI wrappers can stay simple.
...
- Core should not add broad new primitives until Dialog and Select are refactored onto shared internals.
...
- Private internals should not become public subpath exports until a deliberate API decision is made. [ADR 0004](../adr/0004-core-kernel-api-boundary.md) records the current boundary: primitive subpaths, primitive creators, selected utility primitives, form support, and metadata are public support surfaces; generic utilities, overlay kernels, and collection kernels remain private by default.
- Existing utility and primitive files may be reorganized when it reduces duplication and improves testability.
- Kernel modules must remain Keystone-only and must not depend on UI.
...
- Existing Mason registry and CLI tests are useful prior art for deterministic behavior and path-safety testing, but Core needs browser/component tests beyond current package typechecks.

## Parity Tracking Ledger

This ledger is the current module-by-module locality map for Core internals. Status means:

- `proven`: implemented in a shared Core kernel and covered by a Dialog, Select, or focused kernel test.
- `partial`: implemented enough for current Dialog/Select behavior, but still short of the Base UI/Kobalte parity target.
- `missing`: not yet implemented as a shared Core kernel module.
...
| Kernel module                                        | Status | Current Core module                                                                                                            | Proof surface                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Parity gap / next deepening                                                                                                                                             |
| ---------------------------------------------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Controllable state                                   | proven | `packages/core/src/utils/index.ts`                                                                                             | `packages/core/src/utils/kernel.test.tsx` proves controlled/uncontrolled setters, explicit controlled `undefined` values, typed change details, unchanged-value skips, `createDialog`, and `createSelect`; Dialog and Select behavior tests prove open/value changes through public parts.                                                                                                                                                                                                     | Add richer wrappers only when a primitive needs them, for example set-valued state or disclosure helpers.                                                               |
| Event composition and preventable internals          | proven | `packages/core/src/utils/index.ts`, `packages/core/src/overlay/dismissal-policy.ts`                                            | `kernel.test.tsx` proves user handlers run first; `packages/core/test/dialog.behavior.test.tsx` proves prevented outside interaction and prevented Escape; `packages/core/test/select.behavior.test.tsx` proves read-only blocks value changes.                                                                                                                                                                                                                                                | Keep DOM `preventDefault()` as the first cancellation contract. Add shared reason constants only when duplicated reason strings become a maintenance cost.              |
| Solid polymorphic rendering                          | proven | `packages/core/src/utils/index.ts`, Dialog/Select trigger props                                                                | `kernel.test.tsx` proves callback-style `renderPolymorphic`, intrinsic element names, direct Solid components, and router-link-like trigger rendering; `polymorphic.types.tsx` keeps utility, Dialog trigger, Select trigger, and direct Solid component type coverage under `check-types`; `docs/agents/solid-polymorphic-as-rendering.md` records the end-state contract and intentional exceptions.                                                                                         | Base UI render-prop/clone behavior remains intentionally out of scope; callback-style composition is the Keystone escape hatch for explicit final-element prop merging. |
| Stable IDs and SSR guards                            | proven | `packages/core/src/utils/index.ts`, overlay/form/select/dialog creators                                                        | `kernel.test.tsx` proves `createKeystoneId`, the `createStableId` compatibility alias, reactive caller-provided IDs, fallback stability, registered dynamic ARIA IDs, and guarded microtask scheduling; `packages/core/test/accessibility-ssr.test.tsx` proves generated Dialog/FormControl ID relationships without `window` or `document`; Dialog behavior proves ARIA title/description relationships; Select behavior proves trigger/listbox relationships.                                | Keep generated IDs and registered ID stores in the shared utility layer as new primitives add dynamic title, description, and error slots.                              |
| State/data attribute contracts                       | proven | `packages/core/src/utils/index.ts`, `packages/core/src/metadata/index.ts`, form/listbox/overlay/select/dialog                  | `kernel.test.tsx` proves boolean, open/closed, checked/unchecked, and tri-state selection helpers; `packages/core/src/listbox/listbox.test.tsx`, Dialog behavior tests, and Select behavior tests assert `data-scope`, `data-part`, `data-state`, selected/highlighted, readonly, invalid, required, and transition data; `docs/agents/state-data-attribute-helpers.md` records the end-state contract.                                                                                        | Continue documenting new public state attrs when primitives add them.                                                                                                   |
| Portal                                               | proven | `packages/core/src/portal/index.tsx`, overlay `*.Portal` parts                                                                 | `packages/core/src/portal/portal.test.tsx` proves `present`, `forceMount`, custom mount targets, and cleanup; Dialog/Popover/Tooltip/Sheet/Select/Combobox/Menu behavior tests prove the public overlay Portal parts consume the shared helper.                                                                                                                                                                                                                                                | Standalone Portal intentionally owns no ARIA, focus, keyboard, pointer, form, data-attribute, or CSS-variable contract; those remain owned by the portalled content.    |
| Overlay controller                                   | proven | `packages/core/src/overlay/controller.ts`                                                                                      | `createDialog` in `kernel.test.tsx`; Dialog behavior tests for trigger open, close button, Escape, outside dismissal, presence, focus, and nested dialogs.                                                                                                                                                                                                                                                                                                                                     | Keep private until more overlay primitives prove the same API.                                                                                                          |
| Dismissable layer and layer stack                    | proven | `packages/core/src/overlay/layer-kernel.tsx`, `packages/core/src/overlay/dismissable-layer.tsx`                                | `packages/core/src/overlay/layer-kernel.test.tsx` proves layer order, top-layer-only dismissal, pointer blocking, scroll lock, and outside hiding restoration; `packages/core/src/overlay/dismissable-layer.test.tsx` proves outside pointer/focus dismissal, Escape dismissal, preventable handlers, reactive `enabled`, and portalled branch elements; Dialog behavior proves nested dialogs dismiss in top-layer order.                                                                     | Keep validating branch behavior through portal-heavy primitives such as Menu, Combobox, HoverCard, and nested mixed overlay flows as they adopt the shared layer path.  |
| Focus scope, trap, and restore                       | proven | `packages/core/src/overlay/focus-scope.tsx`, consumed by `packages/core/src/overlay/layer-kernel.tsx`                          | Dialog behavior proves initial focus, preventable mount/unmount autofocus, restore focus, non-modal outside focus behavior, Tab wrapping in modal content, top-layer-aware trapping, and last-focused restore when modal focus leaves programmatically. `packages/core/src/overlay/focus-scope.test.tsx` proves focus guards, fallback focus, keyboard wrapping, and cleanup.                                                                                                                  | Keep extending browser-level coverage as portal-heavy primitives such as Menu, Combobox, HoverCard, and nested mixed overlays adopt the shared layer path.              |
| Outside hiding / inert behavior                      | proven | `packages/core/src/overlay/hide-outside.ts`, `packages/core/src/overlay/layer-kernel.tsx`                                      | `layer-kernel.test.tsx` proves `aria-hidden`/native `inert` restoration, dynamically inserted outside DOM, ref-counted cleanup, and top-layer exceptions; Dialog behavior tests prove modal outside content is hidden/inert through the public primitive path.                                                                                                                                                                                                                                 | Keep validating portal-heavy mixed overlay flows and live-announcer exceptions as Menu, Combobox, HoverCard, and nested primitives adopt the shared layer path.         |
| Prevent scroll                                       | proven | `packages/core/src/overlay/prevent-scroll.ts`, `packages/core/src/overlay/layer-kernel.tsx`                                    | `packages/core/src/overlay/prevent-scroll.test.ts` proves body scroll locking, scrollbar compensation, nested lock ref counts, prior inline-style restoration, and iOS touch edge blocking; `packages/core/src/overlay/layer-kernel.test.tsx` proves modal layers acquire and release the shared lock through the public stack path.                                                                                                                                                           | Select/popover-specific scroll behavior should be validated as those primitives adopt modal or scroll-contained variants.                                               |
| Presence and force mount                             | proven | `packages/core/src/overlay/presence.ts`, `packages/core/src/overlay/controller.ts`, overlay `*.Portal` parts                   | `packages/core/src/overlay/presence.test.ts` proves mounted, hidden, transition, close retention, `forceMount`, and `onOpenChangeComplete` retention details; Dialog behavior proves lazy mount, `forceMount` through the public Portal part, close transition retention, `data-transition-status`, and `onOpenChangeComplete`.                                                                                                                                                                | Keep the shared Presence contract private until additional overlay primitives stabilize their public animation APIs.                                                    |
| Floating adapter and geometry variables              | proven | `packages/core/src/overlay/floating.ts`, Select controller                                                                     | `packages/core/src/overlay/floating.test.tsx` proves placement, flip, shift, scroll strategy, sizing, arrow offsets, and CSS variables; Select behavior proves geometry variables on `Select.Positioner`.                                                                                                                                                                                                                                                                                      | Direct Floating UI dependency surface remains private. Deepen collision boundaries and auto-update behavior as Popover/Menu/Combobox adopt it.                          |
| Collection registration and lookup                   | proven | `packages/core/src/collection/collection-registry.ts`, `collection-manager.ts`, `interaction-kernel.ts`, `collection/index.ts` | `kernel.test.tsx` and `packages/core/src/collection/collection.test.tsx` prove ordered registration, duplicate replacement, stale cleanup safety, cleanup on root disposal, grouped options, lookup, DOM-order refs, and hidden-item metadata; Select behavior proves item groups plus dynamic option mount/unmount order through public parts.                                                                                                                                                | Public Listbox subpath and richer virtualization adapters are deferred until a deliberate API decision promotes the private facade.                                     |
| Active descendant, roving focus, and list navigation | proven | `packages/core/src/collection/active-descendant.ts`, `roving-focus.ts`, `keyboard-delegate.ts`, `interaction-kernel.ts`        | `collection.test.tsx` proves `aria-activedescendant`, Arrow navigation, disabled and hidden skips, custom keyboard delegates, grouped options, roving focus tab index/focus movement, and multiple selection contracts; Select, Combobox, and Menu behavior tests prove enabled visible item keyboard navigation through public parts.                                                                                                                                                         | RTL and grid navigation are deferred until primitives require them.                                                                                                     |
| Typeahead                                            | proven | `packages/core/src/collection/typeahead.ts`                                                                                    | `kernel.test.tsx`, `collection.test.tsx`, Select behavior, Combobox behavior, and Menu behavior prove printable-key search, repeated-letter cycling, disabled/hidden skip, Space during active search, and locale-aware custom collator.                                                                                                                                                                                                                                                       | Virtualized offscreen item matching is deferred until a public virtualization adapter exists.                                                                           |
| Selection manager                                    | proven | `packages/core/src/listbox/selection-manager.ts`, Select controller                                                            | `kernel.test.tsx` proves disabled selects are ignored; `listbox.test.tsx` proves single and multiple selection; Select behavior proves keyboard/item selection and repeated hidden inputs for multi-select.                                                                                                                                                                                                                                                                                    | Item equality customization can wait until object-valued or virtualized collections exist.                                                                              |
| Listbox interaction facade                           | proven | `packages/core/src/collection/index.ts`                                                                                        | `collection.test.tsx` proves listbox, option, group, active-descendant, keyboard, typeahead, hidden-item, and selection contracts together; Select, Combobox, and Menu behavior prove public primitives consume the shared kernel.                                                                                                                                                                                                                                                             | Keep private until Listbox/Select/Combobox APIs settle.                                                                                                                 |
| Form-control ARIA and hidden input                   | proven | `packages/core/src/form/index.tsx`, Select controller                                                                          | `kernel.test.tsx` proves label/description/error relationships and hidden input props; `packages/core/src/form/form-control.behavior.test.tsx` proves public `FormControl`/`Field` anatomy, repeated hidden inputs for array values, required/invalid/disabled/readonly ARIA and data contracts, and reset listeners that follow changed external form owners; Select behavior proves form submission, reset, external form owners, input sync, readonly state, and multi-value serialization. | Keep validating custom-control hidden input sync as more non-native field primitives adopt the shared helper.                                                           |
| Field validity                                       | proven | `packages/core/src/form/index.tsx`                                                                                             | `packages/core/src/form/field-validity.test.tsx` proves focus/touch/dirty/filled tracking, native/custom validity, async latest-result behavior, and form-control consumption; `packages/core/src/form/form-control.behavior.test.tsx` proves the public `Field` compound surface consumes validity state for ARIA, data attributes, error message rendering, and native form reset cleanup.                                                                                                   | Field remains Keystone-native and library-agnostic; UI/TanStack Form adapters own app-form integration.                                                                 |

Dialog proof currently lives in `packages/core/test/dialog.behavior.test.tsx`:
...
Select proof currently lives in `packages/core/test/select.behavior.test.tsx`:
...
- Solid API shape: when Base UI's React store, clone/render-prop, or hook mechanics conflict with Solid accessors, lifecycle, or JSX composition, Core follows Kobalte/Solid-native patterns.
...
- Core public contracts: stable `data-scope`, `data-part`, state attributes, and CSS variables are Keystone-owned styling contracts. Inspiration libraries guide coverage, but Core names and lifecycle semantics must stay internally consistent.
- UI and TanStack integrations: TanStack Form/Table/Store/Hotkeys are first-class UI app-layer references, not Core kernel dependencies.
- Sonner-style toast behavior, shadcn-style registry conventions, and UI source-generation ergonomics are first-class UI exceptions and should not be used to justify Core primitive internals.
