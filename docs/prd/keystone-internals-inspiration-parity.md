...
| Solid polymorphic rendering                 | proven  | `packages/keystone/src/utils/index.ts`, Dialog/Select trigger props                                                          | `kernel.test.tsx` proves callback-style `renderPolymorphic` and router-link-like trigger rendering; `polymorphic.types.tsx` keeps utility, Dialog trigger, and Select trigger type coverage under `check-types`.                                                                                                                          | Base UI render-prop/clone behavior remains intentionally out of scope.                                                                                          |
| Stable IDs and SSR guards                   | proven  | `packages/keystone/src/utils/index.ts`, overlay/form/select/dialog creators                                                  | `kernel.test.tsx` proves reactive caller-provided IDs and fallback stability; `packages/keystone/test/accessibility-ssr.test.tsx` proves generated Dialog/FormControl ID relationships without `window` or `document`; Dialog behavior proves ARIA title/description relationships; Select behavior proves trigger/listbox relationships. | A register-id helper for dynamic description/error/title slots is still a future deepening.                                                                     |
| State/data attribute contracts              | proven  | `packages/keystone/src/metadata/index.ts`, utils/form/listbox/overlay/select/dialog                                          | `kernel.test.tsx`, `packages/keystone/src/listbox/listbox.test.tsx`, Dialog behavior tests, and Select behavior tests assert `data-scope`, `data-part`, `data-state`, selected/highlighted, readonly, invalid, required, and transition data.                                                                                             | Continue documenting new public state attrs when primitives add them.                                                                                           |
...
| Dismissable layer and layer stack           | proven  | `packages/keystone/src/overlay/layer-kernel.tsx`, `packages/keystone/src/overlay/dismissable-layer.tsx`                      | `packages/keystone/src/overlay/layer-kernel.test.tsx` proves layer order, top-layer-only dismissal, pointer blocking, scroll lock, and outside hiding restoration; Dialog behavior proves nested dialogs dismiss in top-layer order.                                                                                                      | Branch/layer exceptions are still basic. Deepen before menu/popover nesting grows.                                                                              |
| Focus scope, trap, and restore              | partial | `packages/keystone/src/overlay/focus-scope.tsx`, consumed by `packages/keystone/src/overlay/layer-kernel.tsx`                | Dialog behavior proves initial focus, preventable mount/unmount autofocus, restore focus, non-modal outside focus behavior, Tab wrapping in modal content, top-layer-aware trapping, and last-focused restore when modal focus leaves programmatically.                                                                                   | Missing sentinels and browser-level edge coverage across native Tab order, portals, shadow DOM, and nested mixed overlay primitives.                            |
| Outside hiding / inert behavior             | partial | `packages/keystone/src/overlay/layer-kernel.tsx`                                                                             | `layer-kernel.test.tsx` proves modal siblings receive `aria-hidden` and are restored when layers close.                                                                                                                                                                                                                                   | Missing native `inert`, ref-counted hide outside, mutation-observer coverage, live announcer/top-layer exceptions, and Dialog-level browser coverage.           |
| Prevent scroll                              | partial | `packages/keystone/src/overlay/layer-kernel.tsx`                                                                             | `layer-kernel.test.tsx` proves modal layers set and restore `document.body.style.overflow`.                                                                                                                                                                                                                                               | Missing scrollbar compensation, nested lock ref counts beyond the current stack path, iOS/touch edge handling, and Select/popover-specific scroll behavior.     |
| Presence and force mount                    | partial | `packages/keystone/src/overlay/presence.ts`, `packages/keystone/src/overlay/controller.ts`                                   | Dialog behavior proves lazy mount, close transition retention, `data-transition-status`, and `onOpenChangeComplete`.                                                                                                                                                                                                                      | `forceMount` is wired through Dialog portal but not directly tested. Hidden/unmounted lifecycle should be shared by Popover/Menu/Tooltip before promotion.      |
