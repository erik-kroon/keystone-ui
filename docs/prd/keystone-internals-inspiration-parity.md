...
| Collection registration and lookup                   | proven | `packages/keystone/src/collection/collection-registry.ts`, `collection-manager.ts`, `interaction-kernel.ts`, `collection/index.ts` | `kernel.test.tsx` and `packages/keystone/src/collection/collection.test.tsx` prove ordered registration, duplicate replacement, stale cleanup safety, cleanup on root disposal, grouped options, lookup, DOM-order refs, and hidden-item metadata; Select behavior proves item groups plus dynamic option mount/unmount order through public parts.                                                                                                                                                | Public Listbox subpath and richer virtualization adapters are deferred until a deliberate API decision promotes the private facade.                                     |
| Active descendant, roving focus, and list navigation | proven | `packages/keystone/src/collection/active-descendant.ts`, `roving-focus.ts`, `keyboard-delegate.ts`, `interaction-kernel.ts`        | `collection.test.tsx` proves `aria-activedescendant`, Arrow navigation, disabled and hidden skips, custom keyboard delegates, grouped options, roving focus tab index/focus movement, and multiple selection contracts; Select, Combobox, and Menu behavior tests prove enabled visible item keyboard navigation through public parts.                                                                                                                                                             | RTL and grid navigation are deferred until primitives require them.                                                                                                     |
| Typeahead                                            | proven | `packages/keystone/src/collection/typeahead.ts`                                                                                    | `kernel.test.tsx`, `collection.test.tsx`, Select behavior, Combobox behavior, and Menu behavior prove printable-key search, repeated-letter cycling, disabled/hidden skip, Space during active search, and locale-aware custom collator.                                                                                                                                                                                                                                                           | Virtualized offscreen item matching is deferred until a public virtualization adapter exists.                                                                           |
