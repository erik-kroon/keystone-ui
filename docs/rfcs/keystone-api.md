...
Keystone should expose predictable Solid APIs without copying React-only patterns. The first implementation milestone should prove the API with kernel utilities, `Dialog`, `AlertDialog`, and a small field/control primitive before moving into `Select` and `Combobox`.
...
- Mason registry source conventions beyond the contracts Mason needs to wrap Keystone.
...
Keystone should publish as one public package with subpath exports:

```ts
import { Dialog } from "@keystone-ui/keystone/dialog";
import { createDialog } from "@keystone-ui/keystone/dialog";
import { createControllableSignal } from "@keystone-ui/keystone/utils";
```
...
Creators must be usable without Keystone compound components. Compound components should be built on the same underlying behavior contracts rather than separate logic.
...
This rule lets consumers intentionally prevent Keystone behavior without monkey-patching primitive internals.

## Polymorphism

Keystone should use a Solid-native `as` API.
...
Attributes are part of the public styling contract for design-system wrappers and Mason components.
...
Variables must be scoped to the relevant part and documented on the primitive page. Keystone should not require a global theme provider.
...
Keystone primitives must be SSR-safe.
