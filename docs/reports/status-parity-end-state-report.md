...
- PRDs: `docs/prd/keystone-mason-foundation.md`, `docs/prd/keystone-internals-inspiration-parity.md`.
- ADRs, RFCs, roadmap, accessibility plan, research docs, agent verticals, and readiness audit.
- First-party Mason item parity metadata in `registry/default/items/*.json`.
- Current package manifests and local release verification.

## Executive Summary

The repository is no longer just a scaffold. It is a private `0.1.0` preview workspace with a clear two-layer product model, a broad set of Keystone primitive surfaces, a working Mason registry/CLI tracer, docs metadata, and a passing release verification script.
...
- `bun run test:keystone`: 39 files, 218 tests passed.
- `bun run test:docs`: 3 tests passed, including docs metadata coverage for every Keystone primitive metadata scope and every default Mason registry item.
...
Keystone currently exposes 31 package export entries: the root export plus 30 subpaths. The exported subpaths are:
...
Keystone is the headless, accessible, unstyled primitive layer for Solid. It owns intrinsic behavior: state, focus, keyboard interaction, dismissal, layering, positioning, form semantics, accessibility relationships, SSR safety, and styling contracts.

Mason is the copy-paste source registry and CLI layer. It owns styled wrappers, registry metadata, generated source, blocks, templates, CLI workflows, app-layer integrations, and data-dense workspace patterns.
...
Keystone internals
  -> Keystone primitives
    -> Mason components
      -> Mason app components
        -> Mason blocks
          -> Mason templates
```

TanStack libraries belong in Mason when they are app engines: Form, Table, Store, Hotkeys, Router, and similar systems. Keystone must remain independent from those app libraries.
...
- Package: `@keystone-ui/keystone`, private `0.0.0`.
...
- Tests validate Keystone primitive metadata and Mason registry contracts.

## Keystone Kernel Parity

The strongest current evidence is the parity ledger in `docs/prd/keystone-internals-inspiration-parity.md`. It marks the following internal modules as `proven`:
...
The practical meaning: Keystone now has a credible private primitive kernel. Dialog and Select are the main proof surfaces, with additional proof through Combobox, Menu, overlay derivatives, form-control tests, and focused kernel tests.
...
- Slider covers multi-thumb range state, RTL-aware keyboard/pointer math, hidden inputs, form reset, and Mason styling variables.
- DatePicker/Calendar covers calendar grid behavior, range selection, unavailable dates, locale week starts, keyboard movement, trigger metadata, and Mason wrappers, but remains experimental because date-field depth is not yet there.
...
- Keystone-backed UI: `accordion`, `checkbox`, `collapsible`, `combobox`, `context-menu`, `date-picker`, `dialog`, `dropdown-menu`, `hover-card`, `menu`, `menubar`, `navigation-menu`, `popover`, `radio-group`, `sheet`, `slider`, `switch`, `tabs`, `toast`, `toolbar`, `tooltip`, `autocomplete`.
...
- `tanstackForm`, `tanstackTable`, `tanstackRouter`, `tanstackStore`, and `tanstackHotkeys` for app-layer Mason items.
- `sonner` for toast ergonomics.
- `shadcn` and `mason` for source-registry and utility/block conventions.
...
- Default registry install planning for base components, Keystone-backed dialog, TanStack data-table, TanStack Router adapter, and blocks.
...
- Keystone public primitive APIs: credible but conservatively beta/experimental.
- Mason registry metadata: strong for `0.1.0`.
- Mason generated source: good for local preview and verified tracer apps; needs more app matrices before public confidence.
...
1. Keystone private kernels remain small-interface, deep-behavior modules.
2. Keystone exposes stable compound components and low-level creators only after enough primitives prove the API.
...
7. Mason app components use TanStack app engines instead of custom table/form/store/hotkey systems.
8. Mason grows into data-dense, keyboard-first workspace patterns: realtime tables, watchlists, metric components, command surfaces, resizable shells, chart interaction adapters, condition builders, event feeds, and operational templates.
9. Keystone stays domain-agnostic. Finance or analytics semantics live in Mason blocks, examples, optional packs, or user code.
...
3. Keep Keystone `utils`, overlay internals, Listbox internals, and other kernels private.
...
For `0.2 Mason App Layer`:

1. Deepen DataTable on TanStack Table.
2. Deepen CommandMenu on Keystone Combobox plus TanStack Store/Hotkeys.
3. Build TanStack Form adapters for Mason fields.
...
1. Build workspace-grade Mason blocks only after registry/CLI confidence is higher.
...
2. Add a Keystone/Mason boundary docs page.
...
The repo is in a strong private-preview state by current source evidence, not just by roadmap intent. Keystone already exports a meaningful primitive surface and has substantial behavior/kernel test coverage. Mason already validates and installs a real first-party registry with parity metadata and generated-app verification. The correct next move is not broad catalog expansion. It is public-preview discipline: maturity labels, accessibility evidence, docs, release naming/licensing, Mason lifecycle polish, and selective deepening of the strongest primitive and app-layer proof surfaces.
