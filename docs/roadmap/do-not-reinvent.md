...
Keystone/Mason should focus on primitive behavior, source-owned composition, and data-dense application patterns. It should not spend product energy rebuilding mature engines.

## Keystone Boundary

Keystone may depend on focused primitive infrastructure such as Floating UI. Keystone must not depend on TanStack app-layer libraries.
...
Mason may use app engines directly in generated source. The installed code should stay readable and user-owned.
...
- Floating UI through Keystone primitive behavior, not local Mason positioning code.
