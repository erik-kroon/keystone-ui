...
End-state baseline reached for the Keystone kernel helper layer.
...
- The reusable code was `packages/keystone/src/utils/index.ts`, plus existing Dialog, Select, Listbox, Disclosure, Accordion, Combobox, and Overlay tests that assert observable reasons.
...
- Reason details use `{ event?: Event; reason: string }` through `KeystoneChangeDetail`.
...
- `KeystoneEventHandler` intentionally remains permissive at the type boundary because Solid event handlers encode element-specific `currentTarget` and `target` types that are narrower than the generic kernel can safely name.
...
- The helper remains private to Keystone internals per `docs/agents/keystone-internal-kernel-guidance.md`; no public `./utils` subpath is introduced.
