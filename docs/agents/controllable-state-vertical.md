...
- Existing reusable surface: `createControllableSignal`, `createControllableBooleanSignal`, `KeystoneChangeDetail`, and focused kernel tests in `packages/keystone/src/utils/kernel.test.tsx`.
...
- Keystone primitives continue to expose user-visible roles, ARIA, keyboard, pointer, form, and data attribute behavior through their own public tests; controllable state remains a private kernel helper with focused proof.

## Final Status

Controllable state is `proven` for the Keystone kernel. Keep adding richer wrappers only when a primitive needs them, such as set-valued state or a disclosure-specific helper.
