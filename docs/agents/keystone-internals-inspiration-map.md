...
- [packages/keystone/src/utils/index.ts](../../packages/keystone/src/utils/index.ts): `createControllableSignal` and `composeEventHandlers` are the shared kernel helpers for controlled/uncontrolled state, updater setters, `Object.is` equality skips, explicit controlled presence, and typed change details.
