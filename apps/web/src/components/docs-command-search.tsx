...
import { componentDocs, componentHref, hookDocs, navGroups } from "@/lib/docs-data";
...
export function DocsCommandSearch() {
  ...
  const items = createMemo<SearchItem[]>(() => createSearchItems());
  ...
  return (
    ...
    >
      <Dialog.Trigger
        class="hidden h-10 w-fit min-w-72 cursor-text items-center rounded-full border border-input bg-background px-4 py-2 text-foreground text-sm shadow-xs/5 outline-none transition-[border-color,box-shadow] hover:bg-accent/40 focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/24 md:inline-flex"
        aria-label="Search documentation"
        type="button"
      >
        <span class="flex min-w-0 grow items-center gap-2">
          <Search class="-ms-1 size-5 text-muted-foreground" aria-hidden="true" />
          <span class="truncate font-normal text-muted-foreground/72">Search documentation...</span>
          <span class="pointer-events-none ml-auto inline-flex items-center justify-center font-medium text-muted-foreground/80 text-xs">
        ...
        <Dialog.Positioner class="fixed inset-0 z-50 flex flex-col items-center px-4 py-[max(1rem,4vh)] sm:py-[10vh]">
          <Dialog.Content class="relative flex max-h-105 min-h-0 w-full min-w-0 max-w-xl flex-col rounded-2xl border border-border bg-popover text-popover-foreground shadow-lg/5 outline-none before:pointer-events-none before:absolute before:inset-0 before:rounded-[calc(var(--radius-2xl)-1px)] before:bg-muted/72 before:shadow-[0_1px_--theme(--color-black/4%)] dark:before:shadow-[0_-1px_--theme(--color-white/6%)]">
            ...
            >
              ...
              <div class="relative -mx-px min-h-0 rounded-t-xl rounded-b-2xl border border-border bg-popover bg-clip-padding shadow-xs/5 [clip-path:inset(0_1px_1px_1px_round_0_0_calc(var(--radius-2xl)-1px)_calc(var(--radius-2xl)-1px))]">
            ...
            <div class="flex items-center justify-between gap-2 rounded-b-[calc(var(--radius-2xl)-1px)] border-border border-t px-5 py-3 text-muted-foreground text-xs">
...
function createSearchItems() {
  const overviewItems = navGroups[0]?.items ?? [];
