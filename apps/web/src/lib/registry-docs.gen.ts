...
export const registryDocItems = [
  ...
  },
  {
    ...
    limitations:
      "This source is a local command palette pattern. It does not include fuzzy scoring, async search, nested command pages, command history, user preference persistence, route-specific command discovery, copy-command footer behavior, or a final Hotkeys API guarantee.",
    name: "command-menu",
    parity: {
      visualReference:
  ...
  },
  {
    ...
    sourceFiles: [
      ...
      "packages/ui/src/default/components/data-table/data-table-faceted-filter.tsx",
      "packages/ui/src/default/components/data-table/data-table-pagination.tsx",
      "packages/ui/src/default/components/data-table/data-table-row-actions.tsx",
      "packages/ui/src/default/components/data-table/data-table-skeleton.tsx",
  ...
  },
  {
    ...
    customization:
      "Style generated ui-toast classes, data-slot hooks, position classes, and toast CSS variables while Core owns manager updates, live-region roles, dismissal, timer scheduling, pause/resume, viewport limits, and stable toast data attributes.",
    dependencies: ["@keystone-ui/core", "cn"],
    description:
    ...
    parity: {
      baseUi:
        "Evidence-backed coverage comes through Core manager/provider/store behavior with add, update, dismiss, clear, subscribe, limit, timer scheduling, viewport rendering, action and close parts, roles, and hover/focus pause behavior. UI adds source-owned styling, slots, offsets, positions, and tone affordances without exporting private Core internals. Gaps: measured stacking, promise helpers, cancel/undo policy, and manual live-region AT evidence remain open.",
      kobalte:
        "Solid composition follows Toaster/Region/Toast Root, Title, Description, Close, action-like composition, store subscription, duration, visible limit, pause-on-interaction, and polite/assertive live-region behavior. UI provides the styled copy-paste anatomy and decorative icon slot. Gaps: update timing, pause policy documentation, and manual AT evidence remain follow-up work.",
      sonner:
        "Matches the ergonomic default toaster export, id-based update/dismiss, toast type variants, action callbacks, close button, duration, viewport limit, position offsets, responsive viewport constraints, tone icons, and custom render hook. Swipe, measured stacking, promise helpers, cancel action, and expand mode remain explicit limitations rather than UI reimplementations.",
    },
    registryDependencies: ["cn"],
