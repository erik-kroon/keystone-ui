...
export function Slider(props: SliderProps) {
  const [local, rest] = splitProps(props, ["class", "data-slot"]);

  return (
    <CoreSlider.Root
      {...rest}
      data-slot={local["data-slot"] ?? "slider"}
...
export function SliderTrack(props: SliderTrackProps) {
  const [local, rest] = splitProps(props, ["children", "class", "data-slot"]);

  return (
    <CoreSlider.Track
      {...rest}
      data-slot={local["data-slot"] ?? "slider-track"}
      class={cn(
        "ui-slider-track relative grow cursor-pointer select-none before:absolute before:rounded-full before:bg-input data-[orientation=horizontal]:h-1 data-[orientation=vertical]:h-full data-[orientation=horizontal]:w-full data-[orientation=vertical]:w-1 data-[orientation=horizontal]:before:inset-x-0.5 data-[orientation=vertical]:before:inset-x-0 data-[orientation=horizontal]:before:inset-y-0 data-[orientation=vertical]:before:inset-y-0.5",
        local.class,
      )}
    >
...
export function SliderRange(props: SliderRangeProps) {
  ...
  return (
    <CoreSlider.Range
      {...rest}
      data-slot={local["data-slot"] ?? "slider-indicator"}
      class={cn(
